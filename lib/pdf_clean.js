...
export function generateCustomerPdf({
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Keep total table width < innerW for a right gutter; use a tighter layout.
      // No + Part + Desc + Qty + Price = 28+100+225+50+70 = 473 (w/out cost)
      // With cost, shrink desc so total fits: 28+100+189+50+60+70 = 497
      const noCol   = { key: '__no',  label: 'No',                width: 28,  align: 'right' };
      ...
      const priceCol= { key: 'price', label: 'Unit Price (USD)',  width: 70,  align: 'right' };

      const descWidthNoCost  = 225;
      const descWidthWithCost= 189;
      ...
      const cols = showCost
        ? [noCol, partCol, descCol, qtyCol, costCol, priceCol]
        : [noCol, partCol, descCol, qtyCol,          priceCol];

      const tableW = cols.reduce((s, c) => s + c.width, 0); // 473 (no cost) / 497 (with cost) < innerW
      const headerH = 18;
      const rowPadV = 4;
      const rowGap  = 2;
