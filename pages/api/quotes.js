...
export default async function handler(req,res){
  ...
  const { fields, files } = await new Promise((resolve,reject)=>{ form.parse(req,(err,fields,files)=>{ if(err) reject(err); else resolve({fields,files}); }); });
  ...
  const items = rows.filter(r=>r['Quotation']).map(r=>({ part:r['Parts number']||'', desc:r['Description']||'', qtyTier:r["Q'ty"]||'', price: typeof r['Quotation']==='string'? r['Quotation'] : Number(r['Quotation']).toFixed(2) }));
  const seq = String(Date.now()).slice(-6); const year = new Date().getFullYear(); const quoteId = `Q-${year}-${seq}`; const version=1;
  const quoteDir = path.join(process.cwd(),'storage','quotes',quoteId); const internalDir=path.join(process.cwd(),'storage','internal',quoteId);
  fs.mkdirSync(quoteDir,{recursive:true}); fs.mkdirSync(internalDir,{recursive:true});
  ...
  let txHash=null; try{ const c=getContract(); const tx=await c.logQuoteCreated(quoteId,version,pdfHash,metaHash); const rc=await tx.wait(); txHash=rc.hash; }catch(e){ console.error('Blockchain error:', e.message); }
  ...
  const db = readDB(); db.quotes.push({ quoteId, version, customer, createdAt: Date.now(), pdfHash, metaHash, txHash, secureLink }); writeDB(db);
